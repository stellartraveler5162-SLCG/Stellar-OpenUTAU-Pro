    ...
    public partial class MainWindow : Window, ICmdSubscriber {
        ...
        private void SetupPageTransitions() {
            viewModel.WhenAnyValue(vm => vm.Page).Subscribe(page => {
                ...
                if (carousel == null || carousel.ItemCount < 2) return;
                if (carousel.SelectedIndex == page) return;
                int oldIdx = carousel.SelectedIndex;
                var oldItem = carousel.ItemCount > oldIdx && oldIdx >= 0 ? (carousel.Items[oldIdx] as Control) : null;
                var newItem = carousel.ItemCount > page && page >= 0 ? (carousel.Items[page] as Control) : null;
